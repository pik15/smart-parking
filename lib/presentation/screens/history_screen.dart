...
class HistoryScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFF8F9FE), // Background canvas putih kebiruan sangat lembut
      appBar: AppBar(
        title: const Text(
          'Riwayat Parkir',
          style: TextStyle(
      ...
      body: BlocBuilder<HistoryCubit, HistoryState>(
        builder: (context, state) {
          ...
          return ListView.builder(
            padding: const EdgeInsets.all(16), // Padding luar melonggarkan jarak card ke layar
            itemCount: state.logs.length,
            itemBuilder: (context, index) {
              final log = state.logs[index];
              
              bool isMasuk = log.jenisAksi == "MASUK";
              
              // Menyesuaikan dengan mockup yang menggunakan warna pink transparan & ikon keluar/masuk
              Color warnaIkon = primaryPink;
              IconData ikonAksi = isMasuk ? Icons.login_rounded : Icons.logout_rounded;
                ...
                child: Padding(
                  ...
                  child: ListTile(
                    leading: CircleAvatar(
                      radius: 24,
                      backgroundColor: primaryPink.withValues(alpha: 0.1), // Efek background pink transparan lembut
                      child: Icon(ikonAksi, color: warnaIkon, size: 22),
                    ),
                    title: Text(
                      isMasuk ? 'Kendaraan Masuk Parkiran' : 'Kendaraan Keluar Parkiran',
                    ...
                    trailing: Container(
                      ...
                      decoration: BoxDecoration(
                        color: const Color(0xFFF1F3F9), // Latar abu-abu cerah untuk badge slot
